public AccessLogSamplerBeanInfo() {
    ...
    p.setValue(DEFAULT, "");
    p.setValue(NOT_EXPRESSION, Boolean.TRUE);
    String[] classes = JMeterUtils.loadServicesAndScanJars(Filter.class, ServiceLoader.load(Filter.class), Thread.currentThread().getContextClassLoader(), new LogAndIgnoreServiceLoadExceptionHandler(log)).stream().map(s -> s.getClass().getName()).sorted().toArray(String[]::new);
