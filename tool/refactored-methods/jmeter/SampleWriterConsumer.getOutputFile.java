public File getOutputFile(int channel) {
    String[] fileNameAndExtension = splitFileNameAndExtension(getOutputFile().getName());
    String fName = fileNameAndExtension[0];
    String ext = fileNameAndExtension[1];
    if (channel > 0) {
        fName += "-" + channel;
    }
    return new File(getOutputFile().getParentFile(), fName + "." + ext);
}
