...
public void addFormActionsAndCharSet(String html, Map<? super String, ? super String> formEncodings, String pageEncoding) throws HTMLParseException {
    ...
    for (Element element : forms) {
        ...
        if (!StringUtils.isEmpty(action)) {
            String formCharSet = determineFormCharset(element, pageEncoding);
            if (formCharSet != null) {
...
private static String determineFormCharset(Element formElement, String pageEncoding) {
    // We use the page encoding where the form resides, as the default encoding for the form
    String formCharSet = pageEncoding;
    String acceptCharSet = formElement.attr("accept-charset");
