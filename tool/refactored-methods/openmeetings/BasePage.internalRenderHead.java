protected void internalRenderHead(IHeaderResponse response) {
    renderJQuery(response);
    renderThemeStyles(response);
    renderGoogleAnalytics(response);
    renderFontAwesome(response);
    renderBootstrapResources(response);
    renderCustomStyles(response);
}
