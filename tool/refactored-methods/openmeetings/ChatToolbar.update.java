void update(AjaxRequestTarget target) {
    ...
    chatForm.process(() -> {
        updateVisibility(target, admin, admin);
        return true;
    }, r -> {
        boolean moderator = admin || isModerator(cm, getUserId(), r.getId());
        updateVisibility(target, moderator, moderator);
        return true;
    }, u -> {
        updateVisibility(target, true, true);
        return true;
    });
}
